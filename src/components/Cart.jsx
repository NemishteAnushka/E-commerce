...
import { fetchCart, removeFromCartAsync, updateCartItemAsync } from '../slices/cartSlice';
...
const Cart = () => {
  ...
  const handleUpdateQuantity = async (cartItemId, newQuantity) => {
    ...
    try {
      await dispatch(updateCartItemAsync({ cartItemId, quantity: newQuantity })).unwrap();
    } catch (error) {
      showToast(error.message || 'Failed to update quantity', 'error');
    }
  };

  const subtotal = cart.reduce((total, item) => total + parseFloat(item.product_detail.price) * item.quantity, 0);
    ...
    >
      <Grid container direction={{ xs: 'column', md: 'row' }}>
        ...
        <Grid item xs={12} md={8} sx={{ p: { xs: 2, md: 4 } }}>
          <Typography variant="h5" sx={{ fontWeight: 700, mb: 3 }}>Your Cart</Typography>
          <Divider sx={{ mb: 2 }} />
          <Box
            sx={{
              display: 'flex',
            ...
            {cart.map((item) => (
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  <Paper sx={{ width: 64, height: 64, borderRadius: 2, overflow: 'hidden' }}>
                    <img
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
                  <IconButton onClick={() => handleRemoveFromCart(item.id)}><FaTrash /></IconButton>
                </Box>
              </Box>
            ))}
          </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2, flexWrap: 'wrap' }}>
            ...
            />
            <Button variant="outlined" onClick={() => setDiscount(5)}>Apply</Button>
          </Box>
          <Button variant="text" onClick={() => navigate('/')}>← Continue Shopping</Button>
        </Grid>
